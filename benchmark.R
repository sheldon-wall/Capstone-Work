...
filename <- file.path(dataDir, "tweets.txt")
tweets <- readLines(filename, encoding = 'UTF-8')
...
filename <- file.path(dataDir, "blogs.txt")
blogs <- readLines(filename, encoding = 'UTF-8')
...
{
  scrub_and_predict(x)[1:3]$predictor
}
